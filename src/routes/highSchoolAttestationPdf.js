// High School Attestation PDF Generation Route
// This route generates a PDF from high school attestation certificate data
// Supports both "Attestation de Fréquentation" and "Attestation de Réussite"
...
// POST /api/highschool-attestation-pdf - Generate high school attestation PDF
router.post("/highschool-attestation-pdf", async (req, res) => {
  let browser;
  let page;

  try {
    console.log("🔄 Starting High School Attestation PDF generation...");
    console.log(
    ...
    if (!attestationData) {
      ...
      return res.status(400).json({
        error:
          "Missing attestation data. Please provide valid high school attestation data.",
      });
    }

    // Ensure the attestation data has the correct formType
    if (!attestationData.formType) {
      attestationData.formType = "highSchoolAttestation";
      console.log(
        "🔧 Added formType 'highSchoolAttestation' to attestation data"
      );
    }

    console.log("🎯 High School Attestation Data:", {
    ...
    console.log("💉 Injecting high school attestation data for PDF generation");
    await page.evaluate((data) => {
      ...
      console.log(
        "✅ High school attestation data injected and event dispatched"
      );
    ...
    // Wait for the high school attestation template to be rendered
    console.log(
      "⏳ Waiting for high school attestation template to be ready..."
    );

    const selectors = [
      "#highschool-attestation-template",
      '[data-testid="highschool-attestation-template"]',
      "#bulletin-template",
    ];
    ...
    if (!templateFound) {
      ...
      throw new Error(
        "Could not find high school attestation template on page"
      );
    }

    // Verify template content has loaded
    console.log("⏳ Verifying high school attestation content has loaded...");

    const templateState = await page.evaluate(() => {
      const highschoolAttestationTemplate = document.querySelector(
        "#highschool-attestation-template"
      );
      const bulletinTemplate = document.querySelector("#bulletin-template");
      const template = highschoolAttestationTemplate || bulletinTemplate;
      ...
      return {
        found: true,
        templateType: highschoolAttestationTemplate
          ? "highschool-attestation"
          : "bulletin",
        textContent: textContent.substring(0, 200),
        htmlLength: htmlLength,
        hasHighSchoolAttestation:
          textContent.includes("ATTESTATION") ||
          textContent.includes("FREQUENTATION") ||
          textContent.includes("REUSSITE") ||
          textContent.includes("Préfet"),
    ...
    try {
      await page.screenshot({
        path: "/tmp/highschool-attestation-before-pdf.png",
        fullPage: true,
      });
      console.log(
        "📸 Pre-PDF screenshot saved to /tmp/highschool-attestation-before-pdf.png"
      );
    ...
    const attestationType = attestationData.attestationType || "frequentation";
    const fileName = `highschool-attestation-${attestationType}-${
    ...
    console.error("❌ Error generating high school attestation PDF:", error);

    if (!res.headersSent) {
      res.status(500).json({
        error: "Failed to generate high school attestation PDF",
