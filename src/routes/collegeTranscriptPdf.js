// College Transcript PDF Generation Route
// This route generates a PDF from college annual transcript data
...
// POST /api/college-transcript-pdf - Generate college transcript PDF
router.post("/college-transcript-pdf", async (req, res) => {
  let browser;
  let page;

  try {
    console.log("🔄 Starting College Transcript PDF generation...");
    console.log(
    ...
    const {
      ...
      pdfOptions = {
        format: "A4",
        landscape: false, // Portrait orientation for transcript
    ...
    if (!transcriptData) {
      ...
      return res.status(400).json({
        error:
          "Missing transcript data. Please provide valid college transcript data.",
      });
    }

    // Ensure the transcript data has the correct formType
    if (!transcriptData.formType) {
      transcriptData.formType = "collegeTranscript";
      console.log("🔧 Added formType 'collegeTranscript' to transcript data");
    }

    console.log("🎯 College Transcript Data:", {
      hasStudentName: !!transcriptData.studentName,
      formType: transcriptData.formType,
      courseCount: transcriptData.courses?.length || 0,
    ...
    // Set viewport for portrait orientation rendering (A4 portrait dimensions)
    ...
    // Inject the transcript data into the page (use same pattern as bachelor diploma)
    console.log("💉 Injecting college transcript data for PDF generation");
    await page.evaluate((data) => {
      ...
      window.pdfTranscriptData = data; // Keep for backwards compatibility

      // Dispatch custom event to notify React component
      window.dispatchEvent(
        new CustomEvent("studentDataLoaded", { detail: data })
      );
      console.log("✅ College transcript data injected and event dispatched");
    ...
    // Wait for the college transcript template to be rendered
    console.log("⏳ Waiting for college transcript template to be ready...");

    const selectors = [
      "#college-transcript-template",
      '[data-testid="college-transcript-template"]',
      "#bulletin-template",
    ];
    ...
    if (!templateFound) {
      ...
      throw new Error("Could not find college transcript template on page");
    }

    // Verify template content has loaded
    console.log("⏳ Verifying college transcript content has loaded...");

    const templateState = await page.evaluate(() => {
      const collegeTranscriptTemplate = document.querySelector(
        "#college-transcript-template"
      );
      const bulletinTemplate = document.querySelector("#bulletin-template");
      const template = collegeTranscriptTemplate || bulletinTemplate;
      ...
      return {
        found: true,
        templateType: collegeTranscriptTemplate
          ? "college-transcript"
          : "bulletin",
        textContent: textContent.substring(0, 200),
        htmlLength: htmlLength,
        hasCollegeTranscript:
          textContent.includes("RELEVE") ||
          textContent.includes("TRANSCRIPT") ||
          textContent.includes("MATIERE"),
    ...
    try {
      await page.screenshot({
        path: "/tmp/college-transcript-before-pdf.png",
        fullPage: true,
      });
      console.log(
        "📸 Pre-PDF screenshot saved to /tmp/college-transcript-before-pdf.png"
      );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=college-transcript-${
        transcriptData.studentName?.replace(/\s+/g, "_") || "document"
      }.pdf`
    );
    ...
    if (!res.headersSent) {
      res.status(500).json({
        error: "Failed to generate college transcript PDF",
